...
\@ref(tab:tab-wshd-198692)). Prior to this assessment, there was no fisheries information regarding this stream.
...
[@moe2011Fieldassessment] (Table \@ref(tab:tab-culvert-198692)). The culvert had a substantial outlet drop of 0.3m and an outlet pool with a depth of 0.8m. The water temperature was `r fpr::fpr_my_habitat_info(loc = "ds", col_pull = 'temperature_c')`$^\circ$C,
...
`r fpr_my_survey_dist(loc = 'ds')`. `r fpr_my_habitat_paragraph(loc = 'ds')` The stream was noted as having good flow and containing pockets of gravel suitable for spawning `r if(gitbook_on){knitr::asis_output("(Figure \\@ref(fig:photo-198692-01)).")}else(knitr::asis_output("(Figure \\@ref(fig:photo-198692-d01))."))` Deep pools were infrequent and the morphology was step-pool cobble. The habitat was rated as `r fpr::fpr_my_priority_info(loc = 'ds') %>% stringr::str_to_lower()` value suitable for salmonid rearing and spawning. 
...
- canyon upstream
- observed fish upstream during assessment walk (give distance)

`r fpr_my_survey_dist(loc = 'us')` to a point where gradients increased to 20% `r if(gitbook_on){knitr::asis_output("(Figure \\@ref(fig:photo-198692-02)).")}else(knitr::asis_output("(Figure \\@ref(fig:photo-198692-d01))."))` `r fpr_my_habitat_paragraph(loc = 'us')` There was good flow volume in the area surveyed and step-pool cascades (<1m high) created by large woody debris present throughout and pockets of gravel present suitable for spawning. The habitat was rated as `r fpr::fpr_my_priority_info(loc = 'us') %>% stringr::str_to_lower()` value with moderate rearing potential for dolly varden.
...
Electrofishing was conducted at two sites below and two sites above the crossing with results summarised in Tables \@ref(tab:tab-fish-site-198692) - \@ref(tab:tab-fish-dens-198692) and Figure
\@ref(fig:plot-fish-box-198692). A total of `r tab_fish_summary %>% filter(site_id == paste0(my_site, '_ds')) %>% group_by(site_id) %>% janitor::adorn_totals() %>% filter(site_id == 'Total') %>% pull(count_fish)` dolly varden were captured downstream, and `r tab_fish_summary %>% filter(site_id == paste0(my_site, '_us')) %>% group_by(site_id) %>% janitor::adorn_totals() %>% filter(site_id == 'Total') %>% pull(count_fish)` dolly varden were captured upstream. Multiple fish captured had deteriorating purple gill plates and unusual round black spots `r if(gitbook_on){knitr::asis_output("(Figures \\@ref(fig:photo-198692-03) - \\@ref(fig:photo-198692-04)).")}else(knitr::asis_output("(Figures \\@ref(fig:photo-198692-d02))."))` All fish captured with a fork length greater than 60mm were tagged with Passive Integrated Transponders (PIT tags) with data stored [here](`r paste0(params$repo_url, "blob/main/data/2023/habitat_confirmations.xls")`).
...
- Significant amount of road fill (2.5m) and long culvert (19m) would make construction somewhat involved. 
- substantial outlet drop (0.3m) and lots of fish caught in outlet pool, mean there are definitely fish that could move upstream
- potential for replacement funds from BCTS if they move forward with logging farther down the FSR
- need habitat modelling info
...
```{r photo-198692-04, fig.cap=my_caption2, out.width=photo_width}
knitr::include_graphics(my_photo2)
