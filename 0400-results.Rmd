...
Summary of fish passage assessment procedures conducted in the Peace River Watershed.
...
```{r cost-est-phase-1, eval = T}
 ...
 tab_cost_est_phase1 |>
  select(`PSCIS ID`:`Cost Est ( $K)`) |> 
   fpr::fpr_kable(caption_text = 'Upstream habitat estimates and cost benefit analysis for Phase 1 assessments ranked as a "barrier" or "potential" barrier. Steelhead network model (total length stream network <20% gradient).',
                  scroll = gitbook_on)
```
...
tab_overview |> 
  table_phase2_overview(caption_text = 'Overview of habitat confirmation sites. Bull trout rearing model used for habitat estimates (total length of stream segments <10.5% gradient)', 
                        scroll = gitbook_on)
...
tab_cost_est_phase2 |>
  fpr::fpr_kable(caption_text = 'Cost benefit analysis for Phase 2 assessments. Bull trout rearing model used for habitat estimates (total length of stream segments <10.5% gradient)',
                 scroll = gitbook_on)
