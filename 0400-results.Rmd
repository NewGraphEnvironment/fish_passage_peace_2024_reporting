...
#Make the caption for the rearing model 
...
Fish passage assessment procedures conducted through FWCP in the Peace River Region since 2019 are amalgamated `r if(identical(gitbook_on, FALSE)){knitr::asis_output("online within the Results and Discussion section of the report found  [here](https://newgraphenvironment.github.io/fish_passage_peace_2024_reporting/)")}else{knitr::asis_output("in Tables \\@ref(tab:tab-sites-sum-cap) - \\@ref(tab:tab-sites-cap).")}`
...
In addition to numerous layers documenting fieldwork activities since 2019, a summary of background information spatial layers and tables loaded to the collaborative GIS project (`r params$gis_project_name`) at the
time of writing (`r format(lubridate::now(), "%Y-%m-%d")`) are included 
`r if(identical(gitbook_on, FALSE)){knitr::asis_output("online [here](https://newgraphenvironment.github.io/fish_passage_peace_2024_reporting/)")}else{knitr::asis_output("in Table \\@ref(tab:tab-rfp-tracking).")}`
...
Fish sampling was completed at all habitat confirmation sites as well as several monitoring sites.

<br>

Fish sampling was conducted at `r tab_fish_sites_sum |> n_distinct()` sites within `r tab_fish_sites_sum |> distinct(stream) |> nrow()` streams, with a total of `r tab_fish_summary |> dplyr::pull(count_fish) |> sum(na.rm = TRUE)` fish captured. Fork length, weight, and species were documented for each fish. Salmonids with fork lengths >60mm were PIT-tagged to facilitate long-term tracking of health and movement. Fork length data was used to delineate salmonids based on life stages: fry (0 to 65mm), parr (>65 to 110mm), juvenile (>110mm to 140mm) and adult (>140mm) by visually assessing the histograms presented in Figure \@ref(fig:fish-histogram).  A summary of sites assessed are included in Table \@ref(tab:tab-fish-site-summary) and raw data is provided in [Attachment 3](https://github.com/NewGraphEnvironment/fish_passage_peace_2024_reporting/blob/main/data/fish_data_tags_joined.csv). A summary of density results for all life stages combined of select species is also presented in Figure \@ref(fig:plot-fish-all). 

<br>

Results are presented in detail within the individual habitat confirmation site appendices in this report and in past reports for the monitoring sites, which can be accessed by searching the site number in Table \@ref(tab:tab-sites-cap) and clicking the _Link Report_.
...
All sites with a design for remediation of fish passage can be found in Table \@ref(tab:tab-sites-cap) by filtering using Design = yes. 


## Phase 4 - Remediations

All sites where remediation of fish passage has been completed can be found in Table \@ref(tab:tab-sites-cap) by filtering using Remediation = yes.

<br>

In 2024, remediation of fish passage was completed at PSCIS crossing 125231, located on Tributary to Table River at kilometer 21 on the Chuchinka-Table FSR. The crossing was replaced with a clear-span bridge by C4 with environmental oversight and engineering from DWB Consulting Services Ltd. Partial funding for the project was provided by FWCP through coordination from SERNbc. More information regarding the crossing can be found [here](https://www.newgraphenvironment.com/fish_passage_peace_2023_reporting/).  
...
- Following the 2022 replacement of Tributary to Missinka (PSCIS crossing 125179) on Chuchinka-Missinka FSR, effectiveness monitoring was conducted in 2024. Background can be reviewed [here](https://www.newgraphenvironment.com/fish_passage_peace_2022_reporting/) with results of the monitoring presented in a report appendix.

- Following the 2024 replacement of Tributary to Table (PSCIS crossing 125132) on Chuchinka-Table FSR, effectiveness monitoring was conducted in 2024. Background can be reviewed [here](https://www.newgraphenvironment.com/fish_passage_peace_2023_reporting/) with results of the monitoring presented in a report appendix.
